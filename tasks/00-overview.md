...
- Assets v1: `ETH, WETH, USDC, USDT, WBTC, DAI, cbBTC, wstETH, cbETH`. `ETH<->WETH` = wrap/unwrap edge (1:1, gas only). `HYPE` deferred to v2 + Model B (HyperEVM).
...
## Graph model
- Node = `(chain_id, symbol)` e.g. `(1, USDC)`, `(8453, WETH)`. ~18 nodes v1.
